...
def xuly_khach_hang(uploaded_file):
    try:
        ...
        df_customers = data1[[
            "Số HS", "Họ và tên", "Điện thoại", "Năm sinh", "Giới tính", "Địa chỉ", "Nguồn khách"
        ]]

        # --- Bước 3: Xử lý giới tính và năm sinh ---
        gender_map = {"Nam": 1, "Nữ": 2}
        df_customers["Giới tính số"] = df_customers["Giới tính"].map(gender_map).fillna(3).astype(int)
        df_customers["Năm sinh str"] = df_customers["Năm sinh"].apply(
        ...
        converted["Email"] = ""
        converted["Ngày sinh \n(DD/MM/YYYY hoặc DD-MM-YYYY)"] = df_customers["Năm sinh str"]
        converted["Giới tính \n(1. 1, 2. 2, 3. Khác)"] = df_customers["Giới tính số"]
